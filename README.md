...
- Create a minimal Vite + static HTML setup: basic index.html, package.json with dev/build/preview scripts and Vite as a devDependency. Run npm install so I can use npm run dev. Open a browser window within cursor and use npm run dev.  

**More detailed version with specific github enterprise account details**  
- Create a minimal, production-ready React 18+ application using Vite that can be deployed as a fully static site to an enterprise GitHub environment hosted at https://github.intuit.com/. Use JavaScript (ES6+) and plain CSS (no heavy UI frameworks), with a clean folder structure (/src with main.jsx, App.jsx, /components, /assets, /styles). Configure vite.config.js with the correct base path so the app works when served from the repository subpath used by your enterprise GitHub Pages setup (e.g., /repository-name/), ensure all asset paths are relative, and confirm the production build works correctly under the enterprise GitHub Pages URL structure. Include a responsive layout with a header, main section, footer, and at least one example component. Set up proper npm scripts (dev, build, preview) and deployment via the gh-pages package (with a deploy script pushing /dist to the appropriate Pages branch, such as gh-pages, according to enterprise settings). Generate all required starter files, full code, a proper .gitignore, and a clear README with local setup, build steps, repository creation on github.intuit.com, branch configuration, and enterprise GitHub Pages deployment instructions. The project must run with npm install && npm run dev, build successfully, and deploy without modification within the enterprise GitHub environment.
...
- **Provide rough design direction:**  
